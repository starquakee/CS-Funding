...
public interface FundMapper {
    ...
    @Select("select * from fund f join researchgroup_fund rf on rf.Fund_UUID = f.UUID where f.FundNumber like CONCAT('%', #{FundNumber}, '%') and f.FundName like CONCAT('%', #{FundName}, '%') and f.UUID=#{UUID}}")
    List<Fund> getFundVague(@Param("FundNumber") String FundNumber, @Param("FundName") String FundName, @Param("UUID") String researchGroupId);
